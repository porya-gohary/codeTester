...
public class cppThread extends Thread {
    ...
    public cppThread(mainController controller, List<testCase> testCaseList, String address) {
    ...
    void runCppCode(int index) {
        Platform.runLater(() -> {
            controller.appendToTerminal("Test case #" + (index+1) + "\n");
        });
        ProcessBuilder processBuilder = new ProcessBuilder(address);
